...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a verification plot comparing the historical exact-simulation curve to a fresh ideal-CCZ trajectory aggregate."
    )
    parser.add_argument(
        "--csv",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="append",
        required=True,
        help="Shard directory for the ideal-CCZ trajectory series. Repeat to combine directories.",
    )
    parser.add_argument(
        "--ideal-label",
        default="|H_XY> (Trajectory, ideal CCZs)",
        help="Legend label for the ideal-CCZ trajectory series.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=SCRIPT_DIR / "ler_v2_hxy_ideal_verify.png",
...
def plot(
    ...
    series = [
        ...
        (ideal_traj, ideal_label, "#2ebd85", "#ddf6ea", "s"),
    ...
    if not hide_exact_simulation:
        ...
        series.insert(1, (exact, "Exact simulation", "#008080", "#cfe9e7", "^"))
...
def main() -> None:
    ...
    ideal_traj = aggregate_shard_dirs(args.ideal_traj_dir)
    plot(
        records=records,
